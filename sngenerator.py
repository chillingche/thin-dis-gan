...
#             self.nz, self.ngf * 4 * 4 * 4))  # 512, 4, 4
#         self.main = nn.Sequential(
#             nn.Upsample(scale_factor=2),
#             spectral_norm(
#                 nn.Conv2d(self.ngf * 4, self.ngf * 2, 3, 1, 1,
#                           bias=True)),  # 256, 8, 8
#             nn.BatchNorm2d(self.ngf * 2),
#             nn.ReLU(True),
#             nn.Upsample(scale_factor=2),
...
#             nn.ReLU(True),
#             nn.Upsample(scale_factor=2),
#             spectral_norm(nn.Conv2d(self.ngf, self.nc, 3, 1, 1,
#                                     bias=True)),  # 3, 32, 32
#             nn.Tanh())
...
class SKetchGenerator(nn.Module):
    ...
    def __init__(self, ngpu):
        ...
        self.main = nn.Sequential(
            ...
            nn.ReLU(True),
            nn.Upsample(scale_factor=2),
            SNConv2d(self.ngf * 2, self.ngf, 3, 1, 1,
                     bias=True),  # 128, 16, 16
            nn.BatchNorm2d(self.ngf),
            nn.ReLU(True),
            nn.Upsample(scale_factor=2),
            SNConv2d(self.ngf, self.nc, 3, 1, 1, bias=True),  # 3, 32, 32
            nn.Tanh())
...
class PhotoGenerator(nn.Module):
    ...
    def __init__(self, ngpu):
        ...
        self.ngpu = ngpu
        self.main = nn.Sequential(
            SNConv2d(self.nz + 1, self.ngf, 4, 2, 1, bias=True),  # 128, 16, 16
            nn.LeakyReLU(0.2, True),
            SNConv2d(self.ngf, 2 * self.ngf, 4, 2, 1, bias=True),  # 256, 8, 8
            nn.BatchNorm2d(2 * self.ngf),
            nn.LeakyReLU(0.2, True),
            SNConv2d(2 * self.ngf, 4 * self.ngf, 4, 2, 1,
                     bias=True),  # 512, 4, 4
            nn.BatchNorm2d(4 * self.ngf),
            nn.LeakyReLU(0.2, True),
            nn.Upsample(scale_factor=2),
